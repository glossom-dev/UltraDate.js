...
(function(root, factory) {
  if (typeof define === 'function' && define.amd) {
    ...
    define(['../UltraDate'], factory);
  } else if (typeof module === 'object' && module.exports) {
    // Node. Does not work with strict CommonJS, but
    // only CommonJS-like environments that support module.exports,
    // like Node.
    module.exports = factory(require('../UltraDate'));
  } else {
    // Browser globals (root is window)
    root.UltraDate = factory(root.UltraDate);
  }
}(this, function(UltraDate) {
  'use strict';
  ...
  UltraDate.setFormatOption(consts.strLocale, {
    ...
    longMonth: ['睦月', '如月', '弥生', '卯月', '皐月', '水無月', '文月', '葉月', '長月', '神無月', '霜月', '師走'],
    shortMonth: ['睦月', '如月', '弥生', '卯月', '皐月', '水無月', '文月', '葉月', '長月', '神無月', '霜月', '師走'],
    smallNoon: ['AM', 'PM'],
    largeNoon: ['午前', '午後'],
    era: function(date) {
  ...
  UltraDate.setHolidayOption(consts.strLocale, {
    ...
    setFurikae: function(date, year) {
      ...
      if (date.getDay() === 0 && date >= consts.dateFurikae) {
        while (this.cache[year].hasOwnProperty(date.format(consts.defaultFormat)) || date.getDay() === 0) {
          date.addDate(1);
        }
    ...
    setKokumin: function(date, year) {
      ...
      if (this.cache[year].hasOwnProperty(date.format(consts.defaultFormat)) && date >= consts.dateKokumin) {
